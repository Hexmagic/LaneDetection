...
from models.backbone_xception import Xception
...
class Decoder(nn.Module):
    def __init__(self, shortcut_in_channels, separable=True):
        ...
        self.shortcut = nn.Sequential(
            nn.Conv2d(shortcut_in_channels, 48, 1),
            nn.BatchNorm2d(48),
            nn.ReLU())
    ...
    def forward(self, x, y):
        x = self.shortcut(x)
        # up = self.up(y)
        up = F.interpolate(y, size=x.shape[2:], mode="bilinear", align_corners=False)
...
class DeepLabV3P(nn.Module):
    def __init__(self, n_classes=10, backbone='Xception'):
        ...
        if backbone == 'Xception':
            ...
            raise Exception("deeplab only support backbone Xception or MobileNet")
